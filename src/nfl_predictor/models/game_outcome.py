...
def margin_to_probabilities(
    ...
    home_cover_prob/away_cover_prob are omitted (None) entirely when no
    real market spread_line is available -- there is no "cover" question
    to answer without a real line to cover. A previous version of this
    branch used the model's own predicted_margin as a stand-in spread when
    none was supplied, which is a degenerate choice: P(margin >
    ...
    of how lopsided the model's actual prediction is. Confirmed live on
